...
public class TreeOperate {
	...
	public TreeNode createRandom(int times) {
		TreeNode treeNode = new TreeNode((int) (Math.random() * 100 + 1));
		times--;
		if (times < 0) {
			return null;
		} else {
			treeNode.setLeft(createRandom(times));
			treeNode.setRight(createRandom(times));
		}
		return treeNode;
	}
